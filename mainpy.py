...
# function to  read file directly by choosing a file
def convert_to_speech_from_file1():
...
heading_label.grid(row=0, column=1, pady=10, columnspan=2)

# Create the text entry and label for 'Convert Braille'
text_label2 = tk.Label(window, text="Enter Braille:")
text_label2.grid(row=1, column=1, padx=10, pady=[2,2], columnspan=2)
text_entry2 = tk.Text(window, height=10, width=90)
text_entry2.grid(row=2, column=1, padx=10, pady=[2,10], columnspan=2)

# Create the filename entry and label for 'Convert Braille'
filename_label2 = tk.Label(window, text="Enter Filename:")
filename_label2.grid(row=3, column=1, padx=10, pady=[10,2], columnspan=2)
filename_entry2 = tk.Entry(window)
filename_entry2.grid(row=4, column=1, padx=10, pady=[2,2], columnspan=2)
...
convert_button2.grid(row=5, column=1, padx=10, pady=[2,10], columnspan=2)

# Create a button to trigger the speech conversion
btn_select_file1 = tk.Button(window, text="Select Text File to Read", command=convert_to_speech_from_file1, foreground="white", background="blue")
btn_select_file1.grid(row=6, column=1,padx=10, pady=10, columnspan=2)

# Create the search entry and label
search_label = tk.Label(window, text="Search for Text file:")
search_label.grid(row=7, column=1, padx=10, pady=[10,2], columnspan=2)
search_entry2 = tk.Entry(window)
search_entry2.grid(row=8, column=1, padx=10, pady=[2,2], columnspan=2)

# --------
# --------

# Create a frame for the buttons in the same grid cell
button_frame = tk.Frame(window)

# Set the background color
button_frame.configure(background='light pink')

# Place the frame and buttons using the grid layout
button_frame.grid(row=9, column=1, padx=10, pady=10)

# Create the buttons within the frame
search_button = tk.Button(button_frame, text="Search", command=search_file2, foreground="white", background="dark green")
search_button.grid(row=0, column=0, padx=10, pady=[2,10])
btn_select_file = tk.Button(button_frame, text="Read Text from File", command=convert_to_speech_from_file, foreground="white", background="dark green")
btn_select_file.grid(row=0,column=1,padx=10, pady=[2,10])
...
output_label2.grid(row=10, column=1, padx=10, pady=[10,2], columnspan=2)

# Create the text widget to show the file content
text_widget2 = tk.Text(window, height=10, width=90)
text_widget2.grid(row=11, column=1, padx=10, pady=[2,10], columnspan=2)
